...
export function DraftProtectionLayer({ mode, text, sceneWidth, sceneHeight }: { mode: "off" | "cover"; text?: DraftProtectionTextSettings; sceneWidth: number; sceneHeight: number }) {
  if (mode === "off") return null;
  return <div className={layerStyles.layer} data-testid="draft-protection-layer"><div className="ov-draft-atmosphere" /><BouncingLogo />{text && text.content.trim() && <AnchoredBox layout={text} sceneWidth={sceneWidth} sceneHeight={sceneHeight}><div className={textStyles.text}>{text.content}</div></AnchoredBox>}</div>;
}
