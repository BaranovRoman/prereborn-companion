// WK-116 - ported verbatim (byte-for-byte, no adaptation) from
// apps/web/src/components/pages/stream/queue/red-fog-shaders.ts, since
// this file has zero Next.js/React dependencies - it's pure GLSL wrapped
// in TS template strings. Companion is a separate Vite app with no shared
// package to import web's components from (see AppAtmosphere.tsx's
// history), so this is a copy, not an import - keep it byte-identical to
// the web original on any future shader tuning there, rather than
// re-deriving Companion's own variant.
