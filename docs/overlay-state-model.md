...
2. If Companion is offline or its server-side state is stale, the protected draft
   scene is used. When draft protection is explicitly disabled, the queue scene is
   the safe fallback.
...
runs overlapping requests. React therefore replaces a scene and its complete layout
